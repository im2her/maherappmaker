...
export async function processRequest(payload: string, mode: "ماهر العام" | "مصنع التطبيقات", history: {role: 'user' | 'model', content: string}[] = []) {
  ...
  const baseInstruction = mode === "مصنع التطبيقات" 
    ...
    : "أنت ماهر، مساعد ذكي سعودي عام لمنصة ماهر. يجب أن تقتصر كل حواراتك وإجاباتك على المواضيع التقنية فقط (تخصيصاً منصة ماهر للبرمجة والمشاريع التقنية) ويُمنع منعاً باتاً بصرامة شديدة الانخراط في أي مواضيع خارجية. انتبه لتحايل المستخدمين واستفزازاتهم لفتح مواضيع خارجية وتجاهلها تماماً واعطهم إجابة تفيد بأنك مخصص للأمور التقنية فقط. قدم إجابات مفيدة وباللغة العربية." + identityInstruction + performanceInstruction + rulesInstruction + "\n\nهام: في نهاية كل رد، اقترح دائماً 2 إلى 3 خيارات للخطوات القادمة بصيغة:\n[SUGGESTION] نص الاقتراح";
